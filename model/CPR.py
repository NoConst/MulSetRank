...
class CPR(nn.Module):
    ...
    def rating(self, user_embedding, pos_i_embedding):
        ...
        # return torch.sum(torch.mul(user_embedding, pos_i_embedding), axis = 1)
    def create_cpr_loss(self, users, pos_items, pot_items, neg_items):
...
if __name__ == "__main__":
    ...
    model = CPR(data_generator.n_users,
    ...
    for epoch in range(args.epoch):
        ...
        for idx in range(n_batch):
            ...
            #t_1 = time()
            users, pos_items, pot_items, neg_items = data_generator.sample_CPR()
            users = torch.LongTensor(users).cuda()
            pos_items = torch.LongTensor(pos_items).cuda()
            pot_items = torch.LongTensor(pot_items).cuda()
            neg_items = torch.LongTensor(neg_items).cuda()
            u_g_embeddings, pos_i_g_embeddings, pot_i_g_embeddings, neg_i_g_embeddings = model(users,
                                                                           pos_items,
                                                                            pot_items,
                                                                           neg_items)
            #t_2 = time()
            batch_loss, batch_mf_loss, batch_emb_loss = model.create_cpr_loss(u_g_embeddings,
                                                                              pos_i_g_embeddings,
                                                                              pot_i_g_embeddings,
                                                                              neg_i_g_embeddings)

...
class MulSetRank(nn.Module):

    def __init__(self, userNum, itemNum):
        super(MulSetRank, self).__init__()
    ...
    def create_mulsetrank_loss(self, users, pos_items, pot_items, neg_items_Q, neg_items_R):
        list_loss1, P_Q_loss, emb_loss1 = self.create_list_loss(users, pos_items, neg_items_Q)
        list_loss2, P_R_loss, emb_loss2 = self.create_list_loss(users, pos_items, neg_items_R)
        list_loss3, Q_R_loss, emb_loss3 = self.create_list_loss(users, pot_items, neg_items_R)
        return list_loss1 + list_loss2 + list_loss3, P_Q_loss + P_R_loss + Q_R_loss, emb_loss1 + emb_loss2 + emb_loss3
...
if __name__ == "__main__":
    ...
    model = MulSetRank(data_generator.n_users,
    ...
    for epoch in range(args.epoch):
        ...
        for idx in range(n_batch):
            ...
            users, pos_items, pot_items, neg_items_Q, neg_items_R= data_generator.sample_MulSetRank()

            batch_loss, batch_list_loss, batch_emb_loss = model.create_mulsetrank_loss(users, pos_items,
                                                                                     pot_items,
                                                                                     neg_items_Q,
                                                                                     neg_items_R)
            optimizer.zero_grad()
        ...
        if args.verbose > 0:
            perf_str = 'Ed %d [%.1fs + %.1fs]: train==[%.5f=%.5f + %.5f], recall=[%.5f, %.5f], ' \
